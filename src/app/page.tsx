...
- 14/10 - 16:00 Fernand`);
  ...
  const fetchQuickNotes = useCallback(async () => {
    ...
    try {
      ...
      if (!notesRes.ok) {
        throw new Error(`Erro HTTP! Status: ${notesRes.status}`);
      }

      const notesData = await notesRes.json();

      if (typeof notesData.notes === 'string') {
        setQuickNotes((prevNotes) => (prevNotes === notesData.notes ? prevNotes : notesData.notes));
      }
    } catch (error) {
      console.error('Erro ao carregar notas rápidas:', error);
    }
  }, [isAuthenticated, editingQuickNotes]);
  ...
  useEffect(() => {
    ...
    loadData();
  }, [isAuthenticated, sortRoutineByTime, fetchQuickNotes]);
  ...
  }, [isAuthenticated, fetchQuickNotes]);
